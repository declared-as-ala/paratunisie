...
async function test() {
  ...
  try {
    const res = await service.createShipment('cmtfbr4zy0002pk010d5427jl', {
      weight: 1.0,
      pieces: 1,
      codAmount: 105,
      ville: 'Siliana'
...
with sftp.open("/tmp/test_aramex_label.js", "w") as f:
    f.write(test_script)
sftp.close()

stdin, stdout, stderr = client.exec_command('cat /tmp/test_aramex_label.js | docker exec -i paratunisie-api node')
